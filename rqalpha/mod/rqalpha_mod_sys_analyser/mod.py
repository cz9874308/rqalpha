...
本模块实现了 AnalyserMod 类，是 sys_analyser 模块的主体。
...
1. **数据收集**: 订阅事件，收集每日净值、持仓、成交等数据
2. **基准处理**: 获取基准指数数据用于比较
3. **风险计算**: 使用 rqrisk 库计算各类风险指标
4. **结果输出**: 生成 pickle 文件、CSV 报告、收益图表

数据收集流程
------------

1. PRE_BEFORE_TRADING: 记录每日开始时的状态
2. TRADE: 记录每笔成交
3. POST_AFTER_TRADING: 记录每日结束时的净值和持仓
4. POST_SYSTEM_TEARDOWN: 计算风险指标，输出结果
...
输出的 pickle 文件包含::

    {
        'summary': {...},           # 概要信息
        'portfolio': {...},         # 组合信息
        'benchmark_portfolio': {...}, # 基准组合
        'stock_positions': [...],   # 股票持仓
        'future_positions': [...],  # 期货持仓
        'trades': [...],            # 成交记录
    }
...
- 打击壳价值（2016.11 - 2018.02）
- 公募基金抱团（2020.10 - 2021.03）
- 行业风格切换（2021.09 - 2021.12）
- 小盘踩踏危机（2024.01 - 2024.02）
"""
