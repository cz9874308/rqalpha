...
RQAlpha 采用事件驱动架构，通过 Mod 插件系统实现高度可扩展性：
...
.. code-block:: text
    ...
    │  │                          API 接口层 (apis/)                              │   │
    │  │  ┌────────────┐ ┌────────────┐ ┌─────────────┐ ┌──────────────┐        │   │
    │  │  │order_shares│ │order_target│ │ history_bars│ │get_position  │  ...   │   │
    │  │  └────────────┘ └────────────┘ └─────────────┘ └──────────────┘        │   │
    │  └─────────────────────────────────────────────────────────────────────────┘   │
    │                                      │                                         │
    │                                      ▼                                         │
    │  ┌─────────────────────────────────────────────────────────────────────────┐   │
    │  │                     核心引擎层 (core/ & environment)                     │   │
    │  │                                                                         │   │
    │  │   ┌───────────────┐    ┌───────────────┐    ┌───────────────┐          │   │
    │  │   │  EventSource  │───▶│   Executor    │───▶│   EventBus    │          │   │
    │  │   │  (事件源)      │    │  (事件执行器)  │    │  (事件总线)    │          │   │
    │  │   └───────────────┘    └───────────────┘    └───────────────┘          │   │
    │  │           │                    │                    │                   │   │
    │  │           ▼                    ▼                    ▼                   │   │
    │  │   ┌───────────────┐    ┌───────────────┐    ┌───────────────┐          │   │
    │  │   │  Environment  │◀───│   Strategy    │◀───│  GlobalVars   │          │   │
    │  │   │  (运行环境)    │    │  (策略上下文)  │    │  (全局变量)    │          │   │
    │  │   └───────────────┘    └───────────────┘    └───────────────┘          │   │
    │  └─────────────────────────────────────────────────────────────────────────┘   │
    │                                      │                                         │
    ...
    │  │              │           │              │           │              │       │
    │  │ ┌──────────┐ │           │ ┌──────────┐ │           │ ┌──────────┐ │       │
    │  │ │DataProxy │ │           │ │ Account  │ │           │ │Instrument│ │       │
    │  │ └──────────┘ │           │ └──────────┘ │           │ └──────────┘ │       │
    │  │ ┌──────────┐ │           │ ┌──────────┐ │           │ ┌──────────┐ │       │
    │  │ │DataSource│ │           │ │ Position │ │           │ │  Order   │ │       │
    │  │ └──────────┘ │           │ └──────────┘ │           │ └──────────┘ │       │
    │  │ ┌──────────┐ │           │ ┌──────────┐ │           │ ┌──────────┐ │       │
    │  │ │PriceBoard│ │           │ │Portfolio │ │           │ │  Trade   │ │       │
    │  │ └──────────┘ │           │ └──────────┘ │           │ └──────────┘ │       │
    │  └──────────────┘           └──────────────┘           └──────────────┘       │
    │                                      │                                         │
    │                                      ▼                                         │
    ...
    │  │                                                                         │   │
    │  │  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐ ┌─────────────┐       │   │
    │  │  │sys_accounts │ │sys_simulation│ │ sys_analyser│ │  sys_risk   │       │   │
    │  │  │ (账户管理)   │ │ (模拟撮合)   │ │ (风险分析)  │ │ (风控校验)  │       │   │
    │  │  └─────────────┘ └─────────────┘ └─────────────┘ └─────────────┘       │   │
    │  │  ┌─────────────┐ ┌─────────────┐ ┌─────────────┐                       │   │
    │  │  │sys_scheduler│ │sys_progress │ │sys_trans_cos│  ...用户自定义 Mod    │   │
    │  │  │ (定时任务)   │ │ (进度显示)  │ │ (交易费用)  │                       │   │
    │  │  └─────────────┘ └─────────────┘ └─────────────┘                       │   │
    │  └─────────────────────────────────────────────────────────────────────────┘   │
    │                                                                                 │
...
RQAlpha 的核心是事件驱动机制，每个事件都有 PRE/MAIN/POST 三个阶段：
...
.. code-block:: text
    ...
    └─────────────────────────────────────────────────────────────────────────────┘
        ...
        │  │   │            BEFORE_TRADING 阶段               │   │   │
        │  │   │  ┌─────────────┐  ┌─────────┐  ┌──────────┐ │   │   │
        │  │   │  │PRE_BEFORE_  │─▶│ BEFORE_ │─▶│POST_BEFORE│ │   │   │
...
.. code-block:: text
    ...
    ┌──────────────┐
    │  数据 Bundle  │    H5文件格式存储: 合约信息、日/分钟K线、复权因子等
    │ (~/.rqalpha/ │
    │   /bundle)   │
    └──────┬───────┘
           │
    ...
    │                           DataProxy (数据代理)                           │
    │  ┌────────────────────────────────────────────────────────────────────┐  │
    │  │  门面模式: 封装 DataSource + PriceBoard，提供统一的数据访问接口      │  │
    │  │                                                                    │  │
    │  │  DataProxy = TradingDatesMixin + InstrumentsMixin + ...            │  │
    │  │                                                                    │  │
    │  │  ┌──────────────────┐    ┌──────────────────┐                     │  │
    │  │  │ TradingDatesMixin│    │ InstrumentsMixin │                     │  │
    │  │  │ - 交易日历查询    │    │ - 合约信息查询   │                     │  │
    │  │  │ - 交易日判断      │    │ - 合约历史追溯   │                     │  │
    │  │  └──────────────────┘    └──────────────────┘                     │  │
    │  └────────────────────────────────────────────────────────────────────┘  │
    └──────────────────────────────────┬───────────────────────────────────────┘
           ...
           ▼                           ▼                           ▼
    ┌─────────────┐           ┌──────────────┐           ┌──────────────┐
    │   BarMap    │           │  PriceBoard  │           │  Environment │
    │             │           │              │           │              │
    │ bar_dict    │           │ 最新价格管理  │           │ get_bar()    │
    │ 按合约索引   │           │ 涨跌停价格   │           │ get_last_    │
    │ K线数据      │           │              │           │   price()    │
    └──────┬──────┘           └──────┬───────┘           └──────┬───────┘
           │                         │                          │
           └─────────────────────────┼──────────────────────────┘
    ...
    │       # bar_dict['000001.XSHE'] 获取平安银行K线数据                       │
    │       # history_bars('000001.XSHE', 10, '1d', 'close') 获取历史数据       │
...
.. code-block:: text
    ...
    │
    ├── data/                    # ═══ 数据层 ═══
    │   ├── data_proxy.py        #   数据代理 (门面)
    │   ├── base_data_source/    #   基础数据源实现
    │   ├── instruments_mixin.py #   合约信息混入
    │   └── trading_dates_mixin.py#  交易日历混入
    │
    ├── model/                   # ═══ 领域模型 ═══
    │   ├── instrument.py        #   合约对象
    │   ├── order.py             #   订单对象
    ...
    │
    ├── mod/                     # ═══ Mod 插件 ═══
    │   ├── rqalpha_mod_sys_accounts/      # 账户管理
    │   ├── rqalpha_mod_sys_simulation/    # 模拟撮合
    │   ├── rqalpha_mod_sys_analyser/      # 风险分析
    │   ├── rqalpha_mod_sys_risk/          # 风控校验
    │   ├── rqalpha_mod_sys_scheduler/     # 定时任务
    │   ├── rqalpha_mod_sys_progress/      # 进度显示
    │   └── rqalpha_mod_sys_transaction_cost/  # 交易费用
    │
    ├── apis/                    # ═══ API 层 ═══
    │   ├── api_base.py          #   基础 API
    │   ├── api_stock.py         #   股票 API
    │   └── api_future.py        #   期货 API
    │
    └── utils/                   # ═══ 工具函数 ═══
        ├── logger.py            #   日志工具
        ├── i18n.py              #   国际化
